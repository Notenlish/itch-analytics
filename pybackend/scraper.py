...
from utils import clean_text, get_download_info_from_tags, get_filesize_from_string
...
class Scraper:
    ...
    def scrape_jam_page(self, url: str):
        try:
            ...
            mode = ""
            jam_info_widget = soup.select("div.jam_submitter_widget")[0]
            if soup.select("div.jam_submitter_widget.during_submit"):
                mode = "submit"
            if soup.select("div.jam_submitter_widget.during_voting"):
                mode = "voting"
            if soup.select("div.jam_submitter_widget.after_voting"):
                mode = "over"  # there is also a is_over class but I'm going to act as if I dont know it
            ...
            print(f"Request to {url} got timeout.")
            raise Exception()
        except requests.exceptions.RequestException as e:
            print(f"Got this Exception when sending request to {url}: {e}")
            raise Exception()
            ...
            if field_responses_tag:
                ...
                for response_tag in response_tags:
                    if response_tag.name == "p":
                        ...
                        other_text_exists = (
                            response_tag.text.replace(key, "").strip() != ""
                        )
    ...
    def scrape_game_page(self, url: str):
        try:
            ...
            print(f"Got timeout when fetching {url}")
            raise Exception()
        except requests.exceptions.RequestException as e:
