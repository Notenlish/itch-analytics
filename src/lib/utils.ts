...
export const calculateStandardDeviation = (data: number[], mean: number) => {
  const variance =
    data.reduce((acc, value) => acc + Math.pow(value - mean, 2), 0) / data.length;
...
export const calculateSkewness = (data: number[]) => {
  ...
  const skewness =
    (n * data.reduce((acc, value) => acc + Math.pow((value - mean) / stdDev, 3), 0)) /
    ((n - 1) * (n - 2));
...
export const calculateKurtosis = (data: number[]) => {
  ...
  const kurtosis =
    (n *
      (n + 1) *
      data.reduce((acc, value) => acc + Math.pow((value - mean) / stdDev, 4), 0)) /
      ((n - 1) * (n - 2) * (n - 3)) -
...
export const calculatePointsIntervals = (
  { sortedRatings }: { sortedRatings: number[] },
  { CoolnessByRating }: { CoolnessByRating: number[] }
) => {
...
export const analyzePlatforms = (platformsByRatingNum: string[][]) => {
  ...
  const data = Object.keys(combinationsMap).map(
    (combination) =>
      ({
